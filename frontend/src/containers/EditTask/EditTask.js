...
class EditTaskPage extends Component {
    ...
    componentDidMount() {
        axios.get(TASKS_URL + this.props.match.params.id).then(response => {
            console.log(response.data);
            this.setState(prevState => {
                let newState = {...prevState};
                newState.task = response.data;
                return newState;
            })
        }).catch(error => {
            console.log(error);
    ...
    formSubmitted = (event) => {
        ...
        axios.put(TASKS_URL + this.state.task.id + '/', data).then(response => {
            console.log(response.data);
        ...
        return <Fragment>
            ...
            <div className='container'>
                <form onSubmit={this.formSubmitted}>
                    ...
                    <div className="mt-2">
                        <button type="submit" className="btn btn-success">Отредактировать задачу</button>
                    </div>
                </form>
            </div>
        </Fragment>
